...
<x-slot name="header">
    <h2 class="text-lg md:text-xl font-semibold leading-tight text-gray-800"> <!-- Adjusted text size for smaller screens -->
        {{ __('Edit Note') }}
    </h2>
</x-slot>
...
<div class="py-6 sm:py-12"> <!-- Reduced padding for smaller screens -->
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4"> <!-- Adjusted max-width and padding -->
        <form wire:submit.prevent="saveNote" class="space-y-4"> <!-- Prevent default form submission -->
            ...
            <div class="flex flex-col md:flex-row md:justify-between pt-4"> <!-- Adjusted layout for smaller screens -->
                <x-button type="submit" secondary spinner="saveNote">Save Note</x-button>
                <x-button href="{{ route('notes.index') }}" flat negative class="mt-2 md:mt-0 md:ml-4">Back to Notes</x-button> <!-- Adjusted margin for smaller screens -->
            </div>
